      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl print:hidden">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-white">Vendor Bills</h1>
              <p className="text-xs text-slate-400">Bills created from converted Purchase Orders</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl print:hidden">
          {loading ? (
            <div className="text-center py-16 text-slate-400">Loading vendor bills...</div>
          ) : bills.length === 0 ? (
            <div className="text-center py-16 text-slate-400">No vendor bills found.</div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-800">
                  {bills.map((b) => (
                    <tr key={b.id} className="hover:bg-slate-800/50 transition">
                      ...
                      <td className="px-6 py-4 font-mono text-xs text-slate-400">PO/# {b.purchaseOrderId}</td>
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 print:static print:bg-white print:p-0">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl space-y-6 print:border-none print:shadow-none print:bg-white print:text-black">
              ...
              <div className="flex justify-between items-start border-b border-slate-800 pb-4 print:border-slate-300">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 print:text-slate-600">Official Vendor Purchase Bill</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="text-lg font-black text-violet-400 print:text-violet-700">
              ...
              <div className="grid grid-cols-2 gap-4 bg-slate-950 p-4 rounded-xl print:bg-slate-50 print:border print:border-slate-200">
                <div>
                  <p className="text-[10px] text-slate-400 font-semibold uppercase print:text-slate-600">Vendor:</p>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-300 print:text-black">
                  ...
                  <tfoot className="border-t border-slate-700 font-bold print:border-slate-300">
                    <tr>
                      <td colSpan={3} className="px-4 py-3 text-right uppercase text-slate-400">Total Bill Amount:</td>
                      <td className="px-4 py-3 text-right text-sm font-extrabold text-violet-400 print:text-black">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800 print:hidden">
                <button
                  onClick={handlePrint}
                  className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white rounded-xl text-xs font-semibold shadow transition"
                >
