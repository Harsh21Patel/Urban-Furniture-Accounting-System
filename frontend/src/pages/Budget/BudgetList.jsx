      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-6 rounded-2xl shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-white">Analytical Budget Lifecycle</h1>
              <p className="text-xs text-slate-400">Planned vs Achieved tracking with Draft, Confirmed, Revised, and Cancelled stage flow</p>
            </div>
          </div>
        ...
        <div className="space-y-6">
          {loading ? (
            ...
            budgets.map((b) => (
              <div
                key={b.id}
                className={`bg-slate-900 border rounded-2xl p-6 shadow-xl space-y-6 transition ${
                  b.status === 'REVISED'
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-800">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {b.revisedFrom && (
                        <span className="text-xs text-slate-400 italic">
                          (Revised from #{b.revisedFrom.id}: {b.revisedFrom.name})
                        </span>
                      )}
                    </div>
                  ...
                  {/* Stage Flow Action Buttons (Matching Image 5) */}
                  <div className="flex items-center gap-2">
                    ...
                    {b.status === 'CONFIRMED' && (
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold transition shadow"
                      >
                    ...
                    {b.status !== 'CANCELLED' && b.status !== 'REVISED' && (
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-red-600/80 text-slate-300 hover:text-white text-xs font-semibold transition"
                      >
                ...
                {/* Analytic Line Calculation Table */}
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-800/60 uppercase text-slate-400 font-semibold">
                      <tr>
                    ...
                    <tbody>
                      <tr className="bg-slate-800/30">
                        ...
                        <td className="px-4 py-3 font-semibold text-slate-300">{b.analytic?.type}</td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <button
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded bg-indigo-600/30 hover:bg-indigo-600 text-indigo-300 hover:text-white text-[11px] font-bold transition"
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 max-w-lg w-full shadow-2xl space-y-6 animate-in fade-in zoom-in-95">
              <h2 className="text-xl font-bold text-white">Create New Fresh Budget</h2>
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-white font-bold placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
                  />
                </div>
        ...
        {showReviseModal && selectedBudget && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 max-w-md w-full shadow-2xl space-y-6 animate-in fade-in zoom-in-95">
              <h2 className="text-xl font-bold text-white">Revise Budget: {selectedBudget.name}</h2>
              ...
              <form onSubmit={handleReviseSubmit} className="space-y-4">
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-slate-800">
                  <button
                    type="submit"
                    className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-2.5 rounded-xl shadow-lg transition"
                  >
                    Confirm Revision
                  </button>
                  <button
        ...
        {showTransModal && selectedBudget && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 max-w-2xl w-full shadow-2xl space-y-6 animate-in fade-in zoom-in-95">
              <div className="flex items-center justify-between pb-4 border-b border-slate-800">
                <div>
                  <h2 className="text-xl font-bold text-white">Achieved Transactions Drill-Down</h2>
                  <p className="text-xs text-slate-400">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl border border-slate-700 text-slate-300 hover:bg-slate-800 text-xs font-semibold"
                >
                  Close
                </button>
              </div>
                    ...
                    <tbody className="divide-y divide-slate-800">
                      {budgetTransactions.map((tx, idx) => (
                        <tr key={idx} className="hover:bg-slate-800/50">
                          ...
                          <td className="px-4 py-3 font-mono text-indigo-300">{tx.reference}</td>
