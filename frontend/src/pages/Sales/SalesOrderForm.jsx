...
export default function SalesOrderForm() {
  ...
  const [lines, setLines] = useState([
    { productId: '', quantity: 1, unitPrice: 0, discountPercent: 0, taxPercent: 18, analyticId: '' },
  ]);
  ...
  const discountTotal = lines.reduce((s, l) => {
    ...
    const d = parseFloat(l.discountPercent) || 0;
    return s + q * p * (d / 100);
  }, 0);
  const taxTotal = lines.reduce((s, l) => {
    ...
    const p = parseFloat(l.unitPrice) || 0;
    const d = parseFloat(l.discountPercent) || 0;
    const t = parseFloat(l.taxPercent) || 0;
    const taxable = q * p * (1 - d / 100);
    return s + taxable * (t / 100);
  }, 0);
          ...
          <div className="flex items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400">Select Customer, Products, Quantities, Unit Prices, Discounts, and Taxes</p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Order Line Items</h3>
                <button
                  type="button"
                  onClick={addLine}
                ...
                <table className="w-full text-left text-xs text-gray-700 dark:text-gray-300">
                  <thead className="bg-gray-50 dark:bg-gray-800/60 text-gray-600 dark:text-gray-400 uppercase font-semibold text-[11px]">
                    <tr>
                      ...
                      <th className="px-3 py-2 w-28">Unit Price (Rs.)</th>
                  ...
                  <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                    {lines.map((l, idx) => {
                      ...
                      const t = parseFloat(l.taxPercent) || 0;
                      const taxable = q * p * (1 - d / 100);
                      const lineTotal = taxable * (1 + t / 100);
                            ...
                            >
                              {products.map((prod) => (
                                <option key={prod.id} value={prod.id}>
                                  {prod.name} (₹{Number(prod.salesPrice)})
                                </option>
                              ))}
                            </select>
                          </td>
            ...
            <div className="flex justify-end">
              <div className="w-full sm:w-72 bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-800 rounded-md p-3 space-y-1.5 text-xs">
                ...
                <div className="flex justify-between text-xs font-bold text-gray-900 dark:text-white pt-1.5 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <span className="text-emerald-600 dark:text-emerald-400 font-mono font-extrabold">₹{grandTotal.toFixed(2)}</span>
                </div>
              </div>
            </div>
